import Image from "next/image";
import urlFor from "@/app/lib/urlFor";
import Link from "next-intl/link";
import clsx from "clsx";
import { useTranslations } from "next-intl";
import { HeroData } from "../../../fide/dashboard/components/dashboardUtils";

// helpers locaux
export function formatToBadge(level?: string, classname?: string) {
    if (!level) return null;
    const upper = level.toUpperCase();
    return (
        <span key={level} className={clsx("flex justify-center items-center rounded-lg border border-solid border-neutral-600 px-3 py-1 text-xs text-neutral-100", classname)}>
            {upper}
        </span>
    );
}

export function formatRelative(iso?: string, locale?: "en" | "fr") {
    if (!iso) return "—";
    const d = new Date(iso);
    const diffMs = Date.now() - d.getTime();
    const abs = Math.abs(diffMs);
    const minutes = Math.round(abs / (60 * 1000));
    const hours = Math.round(abs / (60 * 60 * 1000));
    const days = Math.round(abs / (24 * 60 * 60 * 1000));
    if (minutes < 60) return locale === "fr" ? `il y a ${minutes} min` : `${minutes} minutes ago`;
    if (hours < 24) return locale === "fr" ? `il y a ${hours} h` : `${hours} hours ago`;
    return locale === "fr" ? `il y a ${days} j` : `${days} days ago`;
}

type Props = {
    hero?: HeroData | null;
    locale: "fr" | "en";
    hasAccess: boolean;
    courseKey?: string;
};

interface CourseInfo {
    url: string;
    color?: string;
    noAccessImage?: string;
    fr?: {
        title: string;
        denied: React.ReactNode;
        buttonLabel: string;
    };
    en?: {
        title: string;
        denied: React.ReactNode;
        buttonLabel: string;
    };
}

const COURSES_URLS: { [key: string]: CourseInfo } = {
    udemy_course_beginner: {
        url: "/courses/beginners",
        color: "secondary-2",
        noAccessImage: "/images/cours1.jpg",
        fr: {
            title: "Cours Débutant",
            denied: (
                <span>
                    Contenu réservé au <b>Cours Débutant</b>
                </span>
            ),
            buttonLabel: "Acheter ce cours",
        },
        en: {
            title: "Beginner Course",
            denied: (
                <span>
                    Content reserved for the <b>Beginner Course</b>
                </span>
            ),
            buttonLabel: "Buy this course",
        },
    },
    udemy_course_intermediate: {
        url: "/courses/intermediates",
        color: "secondary-4",
        noAccessImage: "/images/cours2.jpg",
        fr: {
            title: "Cours Intermédiaire",
            denied: (
                <span>
                    Contenu réservé au <b>Cours Intermédiaire</b>
                </span>
            ),
            buttonLabel: "Acheter ce cours",
        },
        en: {
            title: "Intermediate Course",
            denied: (
                <span>
                    Content reserved for the <b>Intermediate Course</b>
                </span>
            ),
            buttonLabel: "Buy this course",
        },
    },
    udemy_course_dialogs: {
        url: "/courses/dialogues",
        color: "secondary-5",
        noAccessImage: "/images/cours3.jpg",
        fr: {
            title: "Dialogues du Quotidien",
            denied: (
                <span>
                    Contenu réservé aux <b>Dialogues du Quotidien</b>
                </span>
            ),
            buttonLabel: "Acheter ce cours",
        },
        en: {
            title: "Daily Dialogues",
            denied: (
                <span>
                    Content reserved for the <b>Daily Dialogues</b>
                </span>
            ),
            buttonLabel: "Buy this course",
        },
    },
};

export const VideoCard = ({ hero, locale, hasAccess, courseKey }: Props) => {
    const t = useTranslations("Fide.dashboard.DashboardHero.VideoCard");

    const { main, stats } = hero?.video || {};
    const title = !hasAccess ? t("noAccess") : main?.title || t("emptyTitle");
    const mainImage = (main?.mainImage || main?.mainVideo?.url) as any;
    const levels = (main?.levels as string[] | undefined) || [];

    const statusKey = (main?.status as "watched" | "unwatched" | "in-progress" | undefined) || "unwatched";
    const statusLabel = statusKey ? t(`status.${statusKey}`) : undefined;

    const percentTime = main?.percentTime ?? 0;
    const watched = stats?.watched ?? 0;
    const total = stats?.total ?? 0;
    const lastActivity = stats?.lastActivityAt ? formatRelative(stats.lastActivityAt, locale) : null;
    const isKind = hero?.kind === "video";

    const badges = [...(statusLabel ? [statusLabel] : []), ...levels];

    const href = !hasAccess ? COURSES_URLS?.[courseKey ?? ""]?.url || "#" : `${COURSES_URLS?.[courseKey ?? ""]?.url || ""}/${main?.slug}`;
    const infos = COURSES_URLS?.[courseKey as keyof typeof COURSES_URLS];
    const cardInfo = infos?.[locale];
    const imageSrc = !hasAccess ? infos?.noAccessImage || "/images/placeholder.png" : urlFor(mainImage)?.url() || "/images/placeholder.png";

    return (
        <Link href={href} className="group relative no-underline w-full h-full text-neutral-800 flex flex-col gap-4 px-2 py-4">
            {isKind && <div className={clsx("new-banner py-1 bs", `bg-${infos?.color}`)}>{t("resume")}</div>}

            <div>
                <div className="flex flex-col w-full">
                    <p className={clsx("mb-0 text-3xl underline", `decoration-${infos?.color}`)}>{cardInfo?.title || ""}</p>
                    <p className="text-lg mb-0">{title}</p>
                </div>
            </div>

            <div className="grow w-full">
                {hasAccess ? (
                    <div className="flex items-center justify-between gap-2 w-full">
                        <div className="flex gap-2">{badges.map((lbl) => formatToBadge(lbl, `bg-${infos?.color}`))}</div>
                        <p className="mb-0 text-sm font-bold text-neutral-600">{t("percentWatched", { percent: percentTime })}</p>
                    </div>
                ) : null}
            </div>

            <div className="overflow-hidden w-full rounded-xl flex justify-center bg-neutral-200">
                <Image src={imageSrc} width={300} height={200} alt={t("altVideoThumb")} className="object-cover w-full h-full" />
            </div>

            <div className="flex justify-between items-center">
                <p className="text-sm text-neutral-600 mb-0">
                    <span className="font-bold">
                        {watched}/{total}
                    </span>{" "}
                    {t("views")}
                </p>
                {lastActivity && <p className="text-sm text-neutral-600 mb-0">{t("lastActivity", { time: lastActivity })}</p>}
            </div>

            {!hasAccess && (
                <div className="absolute inset-0 z-10 h-full w-full pointer-events-none [@media(hover:none)]:hidden">
                    <div className="absolute inset-0 bg-neutral-300 opacity-0 transition-opacity duration-300 group-hover:opacity-90" />
                    <div className="absolute inset-0 z-20 flex flex-col items-center justify-center text-center px-4 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                        <Image src="/images/cadenas-ouvert.png" alt={t("lockedOverlayHeadline")} width={64} height={64} className="h-16 w-16 mb-4" />
                        <p className="text-base md:text-xl font-medium">{cardInfo?.denied}</p>
                        <button className="mt-4 btn btn-primary small pointer-events-auto transform transition-transform duration-200 hover:-translate-y-0.5">{cardInfo?.buttonLabel}</button>
                    </div>
                </div>
            )}
        </Link>
    );
};
