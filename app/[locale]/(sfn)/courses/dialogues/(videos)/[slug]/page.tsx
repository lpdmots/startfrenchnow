...
async function DialogsVideosPage(props: { params: Promise<{ locale: string; slug: string }> }) {
    ...
    const hasPermission = !!session?.user?.permissions?.some((p: Permission) => p.referenceKey === "udemy_course_dialogs");
    ...
    if (!isPreview) {
        ...
        const hasPermission = !!session?.user?.permissions?.some((p) => p.referenceKey === "udemy_course_dialogs" && (!p.expiresAt || new Date(p.expiresAt).getTime() > now));

        if (!hasPermission) {
            redirect("/courses/dialogues");
        }
    }

    // 3) TOC pour navigation adjacente
    const fidePackSommaire = await getPackSommaire(locale, "udemy_course_dialogs");
    ...
    return <DialogsVideosPageNoAsync post={localizedPost} previous={previous} next={next} hasPack={hasPermission} fidePackSommaire={fidePackSommaire} locale={locale} />;
}

export default DialogsVideosPage;
...
const DialogsVideosPageNoAsync = ({ post, previous, next, hasPack, fidePackSommaire, locale }: PropsNoAsync) => {
    const previousUrl = previous ? `/courses/dialogues/${previous.slug}` : null;
    const nextUrl = next ? `/courses/dialogues/${next.slug}` : null;
    ...
    return (
        <div className="mb-24 flex w-full flex-col gap-8 md:gap-12">
            <FidePostContent
                ...
                progressType="udemy_course_dialogs"
            />
            <CommentComposer resourceType="udemy_course_dialogs" resourceId={post._id} />
            <CommentList resourceType="udemy_course_dialogs" resourceId={post._id} locale={locale} />
            <BlogLangFixedButton />
        </div>
    );
};
