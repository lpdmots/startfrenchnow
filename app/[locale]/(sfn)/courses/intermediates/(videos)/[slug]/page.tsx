...
async function IntermediateVideosPage(props: { params: Promise<{ locale: string; slug: string }> }) {
    ...
    const hasPermission = !!session?.user?.permissions?.some((p: Permission) => p.referenceKey === "udemy_course_intermediate");
    ...
    if (!isPreview) {
        ...
        const hasPermission = !!session?.user?.permissions?.some((p) => p.referenceKey === "udemy_course_intermediate" && (!p.expiresAt || new Date(p.expiresAt).getTime() > now));

        if (!hasPermission) {
            redirect("/courses/intermediates");
        }
    }

    // 3) TOC pour navigation adjacente
    const fidePackSommaire = await getPackSommaire(locale, "udemy_course_intermediate");
    ...
    return <IntermediateVideosPageNoAsync post={localizedPost} previous={previous} next={next} hasPack={hasPermission} fidePackSommaire={fidePackSommaire} locale={locale} />;
}

export default IntermediateVideosPage;
...
const IntermediateVideosPageNoAsync = ({ post, previous, next, hasPack, fidePackSommaire, locale }: PropsNoAsync) => {
    const previousUrl = previous ? `/courses/intermediates/${previous.slug}` : null;
    const nextUrl = next ? `/courses/intermediates/${next.slug}` : null;
    ...
    return (
        <div className="mb-24 flex w-full flex-col gap-8 md:gap-12">
            <FidePostContent
                ...
                progressType="udemy_course_intermediate"
            />
            <CommentComposer resourceType="udemy_course_intermediate" resourceId={post._id} />
            <CommentList resourceType="udemy_course_intermediate" resourceId={post._id} locale={locale} />
            <BlogLangFixedButton />
        </div>
    );
};
