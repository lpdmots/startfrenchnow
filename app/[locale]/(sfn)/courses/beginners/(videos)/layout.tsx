...
export default async function VideosPostLayout({ children, params: { locale } }: { children: React.ReactNode; params: { locale: Locale } }) {
    ...
    const hasPack = !!session?.user?.permissions?.some((p) => p.referenceKey === "udemy_course_beginner");
    const packSommaire = await getPackSommaire(locale, "udemy_course_beginner");
    ...
    return (
        <div className="grid grid-cols-12 gap-4 xl:gap-8 px-2 sm:px-4 md:px-12 xxl:px-24">
            ...
            <aside className="hidden lg:block lg:col-span-4">
                {
                    <StickyCol>
                        <CoursesAccordionClient fidePackSommaire={packSommaire} hasPack={hasPack} expandAll={false} linkPrefix="/courses/beginners/" />
                    </StickyCol>
                }
            </aside>
        </div>
