...
import { CATEGORIESCOLORSSHADES } from "@/app/lib/constantes";
import { useEffect, useState } from "react";
import { m } from "framer-motion";
import { useExerciseStore } from "@/app/stores/exerciseStore";

export const ProgressBar = ({ questionIndex, exercise }: { questionIndex: number; exercise: ExerciseProps }) => {
    const { getExercise } = useExerciseStore();
    const { questions, score, scoreMax, currentMaxScore } = getExercise(exercise._id);
    ...
    return (
        <div className="flex w-full items-center gap-4 py-2 sm:py-0">
            ...
            <m.div className="w-full rounded-full relative" style={{ height: 5, backgroundColor: CATEGORIESCOLORSSHADES[exercise.category] }}>
                <m.div
